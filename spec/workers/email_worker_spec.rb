...
describe EmailWorker do
  ...
  it "sends an email to the recipient" do
    ItemMailer.should_receive(:update_email).with(item, an_instance_of(Recipient)).once
  ...
  it "sends an email to each recipient" do
    ...
    ItemMailer.should_receive(:update_email).with(item, an_instance_of(Recipient)).twice
